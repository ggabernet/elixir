# nf-core/elixirpipeline: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/elixirpipeline/usage](https://nf-co.re/elixirpipeline/usage)
...
```console
nextflow run nf-core/elixirpipeline --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/elixirpipeline
```
...
First, go to the [nf-core/elixirpipeline releases page](https://github.com/nf-core/elixirpipeline/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
